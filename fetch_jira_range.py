...
OUTPUT_FILE = Path(__file__).resolve().parent / "BIIH_148_to_BIIH_172.md"
START = 148
END = 172
...
def format_issue(issue_key, issue):
...
def main():
    ...
    for num in range(START, END + 1):
        issue_key = f"BIIH-{num}"
        try:
            issue = fetch_issue(base_url, token, issue_key)
            sections.append(format_issue(issue_key, issue))
            print(f"Fetched {issue_key}", file=sys.stderr)
        except requests.HTTPError as exc:
            status = exc.response.status_code if exc.response is not None else "unknown"
            missing.append(f"{issue_key} (HTTP {status})")
            print(f"Missing {issue_key}: HTTP {status}", file=sys.stderr)
        except requests.RequestException as exc:
            missing.append(f"{issue_key} ({exc})")
            print(f"Missing {issue_key}: {exc}", file=sys.stderr)

    content = ["# BIIH-148 to BIIH-172", ""]
    content.extend(sections)
    content.append("## Missing or Inaccessible Tickets")
    content.append("")
    if missing:
        for item in missing:
