...
def sprint_name(field):
    ...
    return ", ".join(names) if names else "ΓÇö"
...
def story_points(fields):
    ...
    return str(pts) if pts is not None and pts != "" else "ΓÇö"
...
def normalize_subtask(base_url, issue):
    ...
    summary = strip_invisible(f.get("summary") or "ΓÇö")
    ...
    return {
        ...
        "priority": (f.get("priority") or {}).get("name") or "ΓÇö",
...
def normalize_story(base_url, issue, subtasks):
    ...
    return {
        ...
        "release": ", ".join(releases) if releases else "ΓÇö",
